...
void FinalFit()
{
  ...
  for (t=1;t<=maxT;t++) // note that Wnext is undefined for t=0 because t=1 if predator has just attacked
  {
    for (h=0;h<=maxH;h++)
    {
      for (d=0;d<=maxD;d++)
      {
        Wnext[t][d][h] = damage_terminal_rewards ? 
                    std::max(0.0, 1.0 - Kfec * double(d))
                    :
                    1.0;
;
      }
      Wnext[t][maxD][h] = damage_terminal_rewards ? 
                    std::max(0.0, 1.0 - Kfec * double(maxD))
                    :
                    0.0; // if no actual accounting for terminal rewards
; // dead if at upper damage limit
    }
  }
}
...
//       << "beta: " << "\t" << beta << endl
       ...
       << "K: " << "\t" << K << endl
       << "Kfec: " << "\t" << Kfec << endl
       << "damage_terminal_rewards: " << "\t" << damage_terminal_rewards << endl
       << "maxI: " << "\t" << maxI << endl
       << "maxT: " << "\t" << maxT << endl
       << "maxD: " << "\t" << maxD << endl
       << "maxH: " << "\t" << maxH << endl
...
int main(int argc, char **argv)
{
    ...
    pAttack = atof(argv[5]);
    Kfec = atof(argv[6]);
    damage_terminal_rewards = atoi(argv[7]);
...
//		outfile_name.str("");
//		outfile_name << "L";
//		outfile_name << std::fixed << pLeave;
//		outfile_name << "A";
//		outfile_name << std::fixed << pArrive;
//		outfile_name << "K";
//		outfile_name << std::fixed << K;
//		outfile_name << "r";
//		outfile_name << std::fixed << repair;
//		outfile_name << "pAtt";
//		outfile_name << std::fixed << pAttack;
//        outfile_name << "rep";
//        outfile_name << std::fixed << replicate;

        string base_name = argv[8];
		string outputfilename = "stress_" + base_name;
        outputfilename += ".txt";
